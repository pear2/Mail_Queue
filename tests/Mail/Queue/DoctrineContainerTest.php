...
class Mail_Queue_DoctrineContainerTest extends Mail_QueueDoctrineAbstract
{
    ...
    public function testGetMailById(){
        ...
        $message = $this->queue->sendMailById($mailId);
     ...
     */
    public function testDeleteMailById(){
        ...
        $delResult = $this->getContainer()->deleteMail($mailId);

        $this->assertEquals($delResult, true);

        $findResult = $delResult = $this->getContainer()->getMailById($mailId);

        $this->assertEquals($findResult, false);
